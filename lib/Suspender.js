...
function Suspender(socket) {
    ...
    this.socket.on('data', (function(data) {
        try {
            ...
            if(!handler) return this.$emit('unhandled', message.c, message.a);
            if(typeof handler != 'function') return this.$emit('error', new Error('Handlers should be functions, got ' + JSON.stringify(handler) + ' instead for ' + message.c));
